public class LinkedList {
    ...
    //creating a function for creation of node
     ...
     //for printing the size of the LL we first create a static variable bkz if wwe want if one changed thn the entire value must change
     public static  int size;//by default th value is initialised as 0
    //creating method to add values to nodes
    public void addfirst(int data) {

         //creating new node
        Node newNode = new Node(data);//creating an object of class node
        size++;//as new node is created
        if(head == null) {
            head = tail = newNode;
            return;
        }
